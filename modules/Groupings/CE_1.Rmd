...
1. Summarize the annual difference in benefits paid and total costs by decade (*summary should include a measure of center and dispersion*). Comment on any patterns over time.
...
1. Construct a data frame that has the sample and mean passer rating for each conference (across both years).
1. Construct a data frame that has the sample size and mean passer rating for each year (across all five conferences).
...
1. Construct a data frame that is like [Table 4.3 (in the link above)](http://derekogle.com/Book107/UnivSum.html#tab:SDCalc) but for the data in `df`. [*Hint: The table will not have the "Sum" row.*]
1. Summarize the data frame created above to compute the standard deviation for these data. [*Hint: this will use `summarize()` and `mutate()` but not `group_by()`. The standard deviation you calculate should equal `r round(sd1$sd,2)`.*]
1. Construct a data frame that is like [Table 4.3 (in the link above)](http://derekogle.com/Book107/UnivSum.html#tab:SDCalc) but will ultimately allow you to compute the standard deviation for both groups. [*Hint: this will require use of `group_by()`.*]
1. Summarize the last data frame created above to compute the standard deviation for both groups in these data. [*Hint: the standard deviations you calculate should equal `r round(sd2$sd[1],2)` and `r round(sd2$sd[2],2)`.*]
